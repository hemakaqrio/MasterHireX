...
const ApplicationList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Extract the IDs of filtered applications
  ...
  const handleScoreChange = async (applicationId: string, newScore: number) => {
    try {
      setUpdatingApplication(applicationId);
      await updateApplicationScore(applicationId, newScore);
      
      // Update application in the state
      setApplications(prev => 
        prev.map(app => 
          app._id === applicationId ? { ...app, score: newScore } : app
        )
      );
    } catch (err) {
      console.error(err);
          ...
          {sortedApplications.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-gray-500">
                {filterStatus === 'filtered'
                  ? "You haven't filtered any applications yet."
